...
def data(filename, nbr_features=None, verbose=False):
    ...
    if verbose: print((np.array(autokit.data_converter.file_to_array(filename))))
    return np.array(autokit.data_converter.file_to_array(filename), dtype=float)
...
def data_sparse (filename, nbr_features):
    ...
    return autokit.data_converter.sparse_list_to_csr_sparse (sparse_list, nbr_features)
