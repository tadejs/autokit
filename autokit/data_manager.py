...
from autokit import data_converter
from autokit import data_io
from autokit.data_io import vprint
import numpy as np
try:
...
class DataManager:
    ...
    def __init__(self, basename, input_dir, verbose=False, replace_missing=True, filter_features=False):
        ...
        if filter_features: # add hoc feature selection, for the example...
            ...
            idx = autokit.data_converter.tp_filter(Xtr, Ytr, feat_num=fn, verbose=verbose)
            Xtr = Xtr[:,idx]
            Xva = Xva[:,idx]
            Xte = Xte[:,idx]  
    ...
    def loadType (self, filename, verbose=True):
        ...
        if os.path.isfile(filename):
            type_list = autokit.data_converter.file_to_array (filename, verbose=False)
        else:
    ...
    def getFormatData(self,filename):
        ...
        if 'is_sparse' in list(self.info.keys()):
            if self.info['is_sparse'] == 0:
                ...
                data = autokit.data_converter.read_first_line (filename)
                if ':' in data[0]:
                    self.info['format'] = 'sparse'
                else:
                    self.info['format'] = 'sparse_binary'
        else:
            data = autokit.data_converter.file_to_array (filename)
            if ':' in data[0][0]:
    ...
    def getNbrFeatures (self, *filenames):
        ...
        if 'feat_num' not in list(self.info.keys()):
            ...
            if self.info['format'] == 'dense':
                data = autokit.data_converter.file_to_array(filenames[0])
                self.info['feat_num'] = len(data[0])
            elif self.info['format'] == 'sparse':
                self.info['feat_num'] = 0
                for filename in filenames:
                    sparse_list = autokit.data_converter.sparse_file_to_sparse_list (filename)
    ...
    def getNbrPatterns (self, basename, info_dir, datatype):
        ...
        line_num = autokit.data_converter.num_lines(os.path.join(info_dir, basename + '_' + datatype + '.data'))
    ...
    def getTypeProblem (self, solution_filename):
        ...
        if 'task' not in list(self.info.keys()):
            solution = np.array(autokit.data_converter.file_to_array(solution_filename))
