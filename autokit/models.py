...
"""
Baseline from AutoML starting kit.
"""
...
class MultiLabelEnsemble:
    ...
    def best_model(self):
        return [pred.best_model() for pred in self.predictors if pred.best_model]

    def retrain_best_model_on_full_data(self, X, Y):
        for i,pred in enumerate(self.predictors):
            pred.retrain_best_model_on_full_data(X, Y[:,i])
    ...
    def fit_iter(self, X, Y):
        ...
        for i in range(self.n_target):
            ...
            if self.balance:
                ...
                ret.append(self.predictors[i].fit_iter(X[chosen,:],Y[chosen,i]))
            else:
                ret.append(self.predictors[i].fit_iter(X,Y[:,i]))

        return Iterators(ret)
    ...
    def predict_proba(self, X):
        ...
        for i in range(1,self.n_target): # More than 1 target, we assume that labels are binary
            pred2 = self.predictors[i].predict_proba(X)
